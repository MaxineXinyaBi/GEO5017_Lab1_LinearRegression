...
- `2_constant_speed_model.py` implements the constant speed model, which assumes the position follows the equation:
  \[
  \text{pos} = \text{initial\_pos} + \text{speed} \times \text{time}
  \]
  The script employs gradient descent to optimize the parameters for each coordinate (\(x, y, z\)) and evaluates the residual errors. The tunable parameters, such as learning rate, iteration limit, and convergence threshold, are marked with comments. Running this script outputs the fitted equations, the number of iterations, and the residual errors.
